...
public class UserFavoritesController : ControllerBase
{
    ...
    // DELETE: api/UserFavorites/delete-by-filter?userId=1&tip=caregiver&title=...
    [HttpDelete("delete-by-filter")]
    public async Task<IActionResult> DeleteByFilter(
        [FromQuery] int userId, 
        [FromQuery] string tip, 
        ...
        [FromQuery] int? targetUserId)
    {
        var query = _context.UserFavorites
            .Where(f => f.UserId == userId && f.Tip == tip);

        if (targetUserId.HasValue)
        {
            query = query.Where(f => f.TargetUserId == targetUserId.Value);
        }
        ...
        {
            return BadRequest("Silinecek öğe için Title veya TargetUserId gerekli.");
        }

        var favoritesToDelete = await query.ToListAsync();
        if (!favoritesToDelete.Any())
        {
            return NotFound("Silinecek favori bulunamadı.");
        }

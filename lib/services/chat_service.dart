...
class ChatService {
  ...
  Future<bool> clearChat(int jobId, {bool deleteConversation = false}) async {
    try {
      final userId = await _getCurrentUserId();
      if (userId == null) return false;

      final response = await httpClient.delete(
        Uri.parse('$baseUrl/clear?jobId=$jobId&userId=$userId&deleteConversation=$deleteConversation'),
      );
  ...
  Future<int?> startConversation(int receiverId) async {
    try {
      final senderId = await _getCurrentUserId();
      if (senderId == null) return null;

      final response = await httpClient.post(
        Uri.parse('$baseUrl/start-conversation'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'senderId': senderId,
          'receiverId': receiverId,

  ...
  void _handleMenuSelection(_ChatMenuAction action) {
    switch (action) {
      case _ChatMenuAction.clearMessages:
        ...
        // WhatsApp gibi: Card'lar kalacak ama içindeki text boş olacak
        setState(() {
          _messages = _messages.map((msg) {
            return ChatMessage(
              ...
              imagePath: null, // Resimleri de kaldır
              imageBytes: null,
            );
          }).toList();
        });
        _saveMessages(_messages, markAsActive: true);

        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text('Mesajlar temizlendi.')));
        break;
